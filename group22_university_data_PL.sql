...
BEGIN
  ...
  WHERE C.universityID = p_universityID;
END //
DELIMITER ;
...
BEGIN
    UPDATE TUITION T
    JOIN COSTS C ON T.costsID = C.costsID
    SET T.currentInStateTuition = p_inStateInput,
        T.currentOutOfStateTuition = p_outStateInput
...
BEGIN
    ...
    INSERT INTO TUITION (currentInStateTuition, currentOutOfStateTuition, costsID)
...
-- drop in FK-safe order
  ...
  CREATE TABLE TUITION (
    ...
    FOREIGN KEY (costsID) REFERENCES COSTS(costID)
      ON DELETE CASCADE
  );
